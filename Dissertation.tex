...
\subsection{Hardware}
move to a more distributed model, with compute nodes, but rather than across a network, within a single computer system. There would always be the need of a central processor for essential management and synchronisation, but the entire system works in a more decentralised manor where an individual component can be programmed independently of other components. In a realistic and practical sense this essentially means having a CPU-like co-processor on the GPU hardware. This would be a huge shift form the current model for both Operating Systems and Program design, but this hypothetical scenario is built to propose a solution to the hardware bottlenecks of device synchronisation.
