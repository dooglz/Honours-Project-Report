...
\begin{abstract}
Emerging technologies like high-resolution displays and virtual reality require significantly more performance than any of the top of the line graphics processing units (GPUs) can provide. Using multiple GPUs may be the solution as computer systems containing multiple GPUs are commonplace among enthusiast gaming and high performance computing industries.
\\
Multiple GPUs can be used in parallel for working on distributed tasks easily, however utilising this power for a single task that requires Inter-GPU communication  (e.g., rendering a frame form a video game) is an unexplored area.
\\
This project investigated and analysed the available methods of GPU communication, and aimed to see if this could be improved with compression.
...
This would be a huge shift form the current model for both Operating Systems and Program design, but this hypothetical scenario is built to propose a solution to the hardware bottlenecks of device synchronisation. 

\section{Conclusion}
The full power and capabilities of GPUs are held back by both hardware and software limitations. At a basic level, GPUs exist physically separate from the CPU and other GPUs, at a complex level GPUs are a very different architecture to the CPU. Controlling them is a task that requires significant work, both on the part of the GPU driver and application developers.

The software APIs available to control GPUs do not provide adequate low-level control over the hardware to undertake significant on-device optimisations like data compression. This is changing for the better as new standards and APIs are being introduced to address this issue.

Communication between two or more GPUs is still uncharted territory, especially for video games. New APIs like DirectX 12 and Vulkan are promoting the use of user defined communication and cooperation between septate devices. This investigation has found this task infeasible with the current generation of available APIs and tools, and suggests that the less the CPU is involved in the process, the faster an application will be. This seems to be the direction of the newer APIs, so hopefully in the near future multiple GPUs can be used together with their full capacity for computation. When this happens, communication speed will become the major issue rather than API computation overhead. This investigation has found that in this situation, theoretically a well tuned compression algorithm could be deployed to leverage spare GPU time to increase data throughput.
